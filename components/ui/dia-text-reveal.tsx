...
> {
  ...
  fixedWidth?: boolean
  /**
   * Externally controlled trigger. When provided, the sweep starts only once
   * `play` becomes `true` (overriding `startOnView`/mount behavior) — e.g. to
   * defer the reveal until an intro overlay is dismissed.
   */
  play?: boolean
  /**
   * When `false`, the sweep plays even if the user prefers reduced motion.
   * Defaults to `true` (honor the OS preference). Set `false` only for
   * user-initiated reveals where the motion is expected.
   */
...
export function DiaTextReveal({
  ...
  // Keep the latest props in a ref so the (recursive) play loop always reads
  // current values. Assigned in an effect rather than during render to satisfy
  // react-hooks/refs.
  useEffect(() => {
    optsRef.current = {
      colors,
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- texts derives from textKey
  }, [textKey, isMulti])
  ...
  useEffect(() => {
    ...
    // `play` (when controlled) overrides the view/mount gate.
    const triggered = play !== undefined ? play : !startOnView || isInView
    if (!triggered) return
  ...
  }, [isInView, startOnView, once, reduced, sweepPos, play])
