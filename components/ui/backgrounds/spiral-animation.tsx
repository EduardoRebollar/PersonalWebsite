...
// Adapted from the 21st.dev `spiral-animation` snippet. GSAP-driven
// canvas animation: a long camera dolly through a swirl of ~5k particles.
// SSR-safe — `window` is only touched inside effects.
...
class AnimationController {
  ...
  // Seeded RNG used only while constructing the star field so the swirl
  // pattern is reproducible across reloads. Restored to native immediately.
  ...
  // Public accessors so the Star class can compose 3D positions without
  // reaching into private fields (strict TS, no bracket-string access).
