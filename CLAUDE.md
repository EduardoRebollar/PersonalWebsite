...
components/
  ...
  ui/                Grouped into subfolders (all import via the
    ...
    (ui/ root)         Section-specific widgets kept flat: timeline (Journey),
                       ...
                       floating-text + text-effect (Hero intro reveal;
                       dia-text-reveal is the superseded name-sweep), Lightbox
  ...
  laHistory/         Native Next.js port of LA History: Leaflet map, Cytoscape concept map,
    ...
    casestudy/         Bespoke editorial "broadsheet" case study for /work/la-history —
                         ScrollExpandCover (scroll-hijack cover), CaseStudyShell (.anim-ok gate),
                         SectionRail, animation primitives (Reveal, SplitText, WhisperText,
                         ScrollRevealWords, ShimmerText), TutorTranscript, ZoomParallax,
                         Gazetteer, ResultsBars, DustMotes, icons. Styles scoped in
                         styles/casestudy.css (.lah-cs)
  seo/               JsonLd
content/
...
- **Hero entrance choreography**: gated on `splashDismissed`, the left-column copy animates in as a sequence — the display name reveals via `FloatingText` (`components/ui/floating-text.tsx`: per-letter indigo→white rise, then a continuous bob + a periodic Matrix-glyph *decode* scramble; reduced-motion reveals the letters then holds them still), the eyebrow + tagline fade in per-character via `TextEffect`, then the CTAs cascade up (staggered `motion` variants) with `ScrollHint` (`play` prop) as the final beat. The *See work* CTA is a `ShinyButton`; the ghost *Get in touch* link carries `.cta-glow` — a pulsing white text-shadow halo (`glow-pulse` keyframe, top-level in `globals.css`). Radii stay under the button padding so `RippleLink`'s `overflow-hidden` doesn't clip the halo. The cascade was light-trimmed in Phase 12 to settle ~3.4 s after Enter (was ~4.5 s); the dials are `HERO_CASCADE_DELAY` (the post-warp hold in `SpiralSplash.tsx`) plus the per-beat `delay` / `delayChildren` in `Hero.tsx` and `ScrollHint.tsx`.
...
- **LA History broadsheet case study**: `/work/la-history` branches in `app/work/[slug]/page.tsx` to a bespoke full-bleed editorial page (`components/laHistory/casestudy/LaHistoryCaseStudy.tsx`), separate from the MDX case studies. All of its CSS is scoped under `.lah-cs` in `components/laHistory/styles/casestudy.css` (keyframes use the `bs*` camelCase convention, e.g. `bsShimmer` / `bsArrowBob` / `bsCoverArrowBob`). `CaseStudyShell` adds `.anim-ok` to the root before paint **only when motion is allowed** — every entrance/loop is gated on `.lah-cs.anim-ok …`, so reduced-motion / no-JS visitors get the static, settled state for free (no per-component reduced-motion branch needed). The page opens with `ScrollExpandCover` — a scroll-hijack cover that grows a muted autoplay video to fill the screen, then releases into the article (keyboard-advanceable so the hijack isn't a trap; falls back to a static framed still under reduced-motion). A fixed `SectionRail` tracks reading position. Text animates via a small set of primitives: `Reveal` (intersection fade/stagger), `SplitText` / `WhisperText` / `ScrollRevealWords` (per-word/char reveals), and `ShimmerText` — the reusable teal "shine" (a white glint band sweeping over a `background-clip: text` accent fill via `.bs-shimmer` + `@keyframes bsShimmer`; `ShimmerText` only times it, adding `.is-shimmering` on scroll-in and pausing offscreen, per the site's offscreen-pause convention). For a green shine, override `--shimmer-base: var(--accent)` (default base is grey). Continuous up/down "bob" arrows (the cover's green down-arrow, the `TutorTranscript` step button's arrow) are pure CSS `translateY` keyframes applied directly — **not** hover-gated — and freeze automatically under the global reduced-motion clamp. `TutorTranscript` is a live, steppable Socratic-tutor transcript (state machine, per-word tutor reveal, typing dots).
...
- **`next/dynamic({ ssr: false })` causes SSR subtree bailout in Next 16.** Use manual `lazy()` + `<Suspense>` inside a `'use client'` component instead. See `components/ui/three/SplineScene.tsx`. And `{ ssr: true }` (the default) does **not** defer hydration: a server component that `dynamic`-imports a `'use client'` section still ships that section's JS in an **eager** initial `<script>` chunk (verified Phase 12 — the split was reverted). Deferring below-the-fold section hydration needs true viewport-gated lazy mounting, not `next/dynamic`.
- **React 19 Float auto-preloads every server-rendered eager `<img>`.** A raw `<img src>` (not `next/image`) rendered on the server makes React emit a `<link rel="preload" as="image">` for it — so the ~30 below-the-fold brand icons in `orbiting-skills.tsx` all preloaded at startup (~70.5 KB of contention) until they got `loading="lazy" decoding="async"` in Phase 12. **Any new below-the-fold raw `<img>` must be `loading="lazy"`** or it will preload. (`next/image` already lazies by default.)
- **Turbopack always ships the `nomodule` core-js polyfills bundle** (~41 KB br, a hashed chunk) regardless of `browserslist` — the modern `browserslist` now pinned in `package.json` (Chrome/Edge/FF ≥111, Safari/iOS ≥16.4) does **not** drop it. It doesn't cost the score: modern browsers never download `nomodule` scripts. The browserslist still earns its keep by trimming transpilation in the *module* chunks (~48 KB lighter initial JS).
...
| Phase 12 — PageSpeed perf pass: `loading="lazy" decoding="async"` on the 30 below-the-fold brand `<img>` in `orbiting-skills.tsx` (kills React 19 Float's auto-preload — **−70.5 KB / 30 requests** off the initial-load critical path); modern `browserslist` in `package.json` (Baseline-2023, trims module-chunk transpilation); light-trimmed the post-Enter Hero reveal cascade (~4.5 s → ~3.4 s, in `Hero.tsx` / `SpiralSplash.tsx` / `ScrollHint.tsx`). Tested **and reverted** a `next/dynamic` below-the-fold section split (proved inert — `ssr:true` doesn't defer hydration). Caveat: the Enter-gated splash means Lighthouse measures the splash, not content, so lab LCP/TBT stay splash-dominated — user chose to keep the Enter-only gate (no auto-reveal) | ◧ working tree, not yet committed |
| Phase 13 — LA History broadsheet case study: bespoke editorial `/work/la-history` page (`components/laHistory/casestudy/*`, scoped `.lah-cs` in `styles/casestudy.css`) branched in `app/work/[slug]/page.tsx`. `ScrollExpandCover` scroll-hijack cover (video grows to fill, releases into the article; keyboard-advanceable; static fallback under reduced-motion); `CaseStudyShell` `.anim-ok` motion gate; `SectionRail` reading-position rail; text-animation primitives (`Reveal` / `SplitText` / `WhisperText` / `ScrollRevealWords` / `ShimmerText` teal shine); live steppable `TutorTranscript`; `ZoomParallax` screenshot montage; `Gazetteer` / `ResultsBars` / `DustMotes`. Interaction polish: cover hint uses `ShimmerText` (green shine, `--shimmer-base: var(--accent)`) + a continuously-bobbing green down-arrow; the `TutorTranscript` step button's arrow bobs continuously (CSS `bsArrowBob` / `bsCoverArrowBob`, frozen by the global reduced-motion clamp). Interface screenshots are still placeholders | ◧ working tree, not yet committed |

## Known follow-ups (not blocking launch)

- **Custom domain** (currently `eduardorebollar.vercel.app`).
- **Recorded demo / screenshots** for LA History — the broadsheet's `ZoomParallax` montage references `public/la-history/screens/*.webp` (welcome / map / quiz / concept-map / dashboard / breakdown / gallery / build-map), but these are still placeholders awaiting real captures of the live app.
...
- **Defer below-the-fold section hydration** — all six sections hydrate on initial load (the main TBT cost, and it runs behind the splash). `next/dynamic({ ssr: true })` does **not** help (Phase 12 confirmed the chunk stays eager); the real lever is true viewport-gated lazy hydration, which is complex/risky — only worth it against a measured Lighthouse baseline. The PSI keyless API is globally 429-throttled, so measure via pagespeed.web.dev manually or local `npx lighthouse`.
...
- **All ~30 brand icons preload at once on first load (PageSpeed flags startup contention)?** The raw `<img src="/brand/*.svg">` in `orbiting-skills.tsx` must keep `loading="lazy"` — React 19 Float auto-preloads any *eager* server-rendered `<img>`, even below the fold. Check the served HTML's `<head>` for `<link rel="preload" as="image" href="/brand/…">`; if they're back, a brand `<img>` lost its `loading="lazy"`.
- **A LA History case-study animation (shimmer, reveal, bob) doesn't run even with motion enabled?** All of it is gated on `.lah-cs.anim-ok …`. `CaseStudyShell` only adds `.anim-ok` when `prefers-reduced-motion` is *not* set — toggle via DevTools Rendering → "Emulate prefers-reduced-motion: no-preference" to confirm. The `ShimmerText` glint additionally needs `.is-shimmering` (added on scroll-in, removed offscreen) — if a shimmer shows the solid base color but never sweeps, it's offscreen per the observer or the delay hasn't elapsed.
- **`ShimmerText` shows the wrong base color (grey, not the teal accent)?** The default `--shimmer-base` is `var(--fg-mute)` (grey). Teal labels must opt in — `.lbl.bs-shimmer` does it globally; for one-off cases (e.g. the cover hint) set `--shimmer-base: var(--accent)` on a wrapping rule.
- **The LA History cover traps scroll / won't release into the article?** `ScrollExpandCover` hijacks wheel/touch until the media fully expands; it deliberately releases (and skips the hijack) when `window.scrollY > 5` on load (deep reload/restore) or on an in-app SPA nav (`routeState.spaNavigated`). Keyboard users advance via Arrow/PageDown/Enter/Space. Reduced-motion never engages the hijack at all (static still).
